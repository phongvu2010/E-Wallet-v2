...
CREATE TYPE account_status_enum AS ENUM ('ACTIVE', 'LOCKED', 'CLOSED', 'EXPIRED');
...
CREATE TYPE transaction_type_enum AS ENUM (
    ...
    'INSTALLMENT_PRINCIPAL', -- Trừ số tiền gốc khi chuyển đổi trả góp (ghi có âm)
...
CREATE TABLE accounts (
    ...
    card_number_masked VARCHAR(25) NOT NULL, -- "4696 72xx xxxx 2958"
    card_number_last4 VARCHAR(4) NOT NULL, -- "2958", "0642", "0702"
    currency VARCHAR(3) DEFAULT 'VND',
    credit_limit DECIMAL(15, 2) DEFAULT 0.00, -- Hạn mức tín dụng (VND)
    billing_day_of_month INT CHECK (billing_day_of_month BETWEEN 1 AND 31), -- Ngày chốt sao kê (ví dụ: ngày 20)
...
-- ====================================================================
-- 4. MERCHANTS (ĐƠN VỊ CHẤP NHẬN THẺ)
-- ====================================================================
CREATE TABLE merchants (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    raw_name VARCHAR(255) NOT NULL, -- "PAYOO*MINISTOP 8258 Q TAN BINH VN"
    cleaned_name VARCHAR(100) NOT NULL, -- "Ministop"
...
CREATE TABLE installment_plans (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    account_id UUID NOT NULL REFERENCES accounts(id) ON DELETE CASCADE,
    origin_transaction_id UUID REFERENCES transactions(id) ON DELETE SET NULL, -- Giao dịch gốc được chuyển đổi
...
-- 10. ANALYTIC VIEWS
-- ====================================================================
-- View 1: Thống kê chi tiêu theo tháng và danh mục
...
WHERE t.transaction_type IN ('PURCHASE', 'INSTALLMENT_MONTHLY', 'FEE', 'INTEREST')
GROUP BY 1, 2, 3
