...
def ensure_database_schema(cursor, connection):
    """Ensure essential coordination tables, debt origin column, and live views exist.

    This runs ONCE during data migration / DB initialization, preventing concurrent DDL
    in Gunicorn multi-workers.
    """
    print("[ETL Ingestion] Đang kiểm tra cấu trúc database & bảng điều phối (Schema compatibility)...")
    cursor.execute("""
...
try:
    ...
    # 0. Ensure Schema Compatibility & Coordination Tables
    ensure_database_schema(cur, conn)
